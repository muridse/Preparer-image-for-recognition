...
namespace Recognizer
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            var clear = MedianFilter.MedianFilterRun(grayscale);
            panel.Controls.Add(CreateBox(ConvertToBitmap(clear)), 2, 0);
            var sobell = SobelFilter.SobelFilterRun(clear, new double[,] {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}});
            panel.Controls.Add(CreateBox(ConvertToBitmap(sobell)), 0, 1);
            var trashhold = ThresholdFilter.ThresholdFilterRun(sobell, 0.1);
            panel.Controls.Add(CreateBox(ConvertToBitmap(trashhold)), 1, 1);
            //var buityfier =  BuityfierTask.BuityfyOriginalWithSobel(pixels, sobell);
            //panel.Controls.Add(CreateBox(ConvertToBitmap(buityfier)), 0, 0);
